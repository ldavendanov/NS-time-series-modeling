...
for na = 1:na_max
    ...
    order = [na pa];
    [A,sigmaW,~,criteria] = lpv_ar(x(ind_train),xi(ind_train),order,options);
    rss_sss(1,na) = criteria.rss_sss;
    lnL(1,na) = criteria.lnL;
    bic(na) = criteria.bic;
    
    %-- Calculating validation performance criteria
    [~,criteria] = simulate_lpv_ar(A,sigmaW,x(ind_val),xi(ind_val),order,options);
...
[A0,sigmaW0,SigmaTh,criteria] = lpv_ar(x(ind_train),xi(ind_train),order,options);
rss_sss(1,1) = criteria.rss_sss;
lnL(1,1) = criteria.rss_sss;

%-- Validating the LPV-AR model on the validation data
[xhat0,criteria] = simulate_lpv_ar(A0,sigmaW0,x(ind_val),xi(ind_val),order,options);
...
sigmaTh = reshape(sigmaTh,pa,na);                                           % Variance of the LPV-AR coefficients
...
pt = zeros(pa,1);
for j=1:pa
    pt(j) = semilogy( 1:na, chi2(j,:), ['--',Mrk(j)], 'MarkerSize', 10, 'LineWidth', 2 );
    hold on
end
