function [xttN,PttN,other] = ExtendedKalmanSmoother(y,State,Covariance,System)
%--------------------------------------------------------------------------
% Function compute the Extended Kalman smoother estimates given the Kalman
% filter estimates xh[t], using the following SS model
%       x[t] = F(x[t-1]) + G*w[t]
%       y[t] = H(x[t]) + e[t]
% where 
...
n = size(System.F(xtt(:,1)),1);     % Size of the state vector
xttN = zeros(size(xtt));
PttN = zeros(size(Ptt));
...
for t=N-1:-1:1
    ...
    % Smoothing equations
    A(:,:,t) = Ptt(:,:,t)*F'*pinv(Pttm(:,:,t+1));
    xttN(:,t) = xtt(:,t) + A(:,:,t)*( xttN(:,t+1) - xttm(:,t+1) );
    PttN(:,:,t) = Ptt(:,:,t) + A(:,:,t)*( PttN(:,:,t+1) - Pttm(:,:,t+1) )*A(:,:,t)';
