function [theta_hat,P_theta,y_hat,HyperPar,other] = SPTAR_MLII(y,na,q,options)
%--------------------------------------------------------------------------
...
[theta0,sw0] = arburg(y(1:100),na);                 % Initial parameter mean and innovations variance values
theta0 = theta0(2:end)';
...
problem.objective = @(x)SPTARlogMarginal(y,na,q,x);
...
HP.theta0 = HyperPar(1:na);                         % Optimized parameter mean
HP.sigma_w2 = 10^HyperPar(end-1);                   % Optimized innovations variance
HP.Sigma_v = (10^HyperPar(end))*eye(na);            % Optimized parameter innovations variance
[theta_hat,P_theta,y_hat,other] = GSCTAR_MAPtrajectory(y,na,HP,options);

%--------------------------------------------------------------------------
function [logL,theta_hat,P_theta,y_hat,other] = SPTARlogMarginal(y,na,q,HP)

% Obtaining information from the input
mu0 = poly(ones(1,q));
mu0 = mu0(2:end);
Hyperpar.mu = mu0;
Hyperpar.theta0 = HP(1:na);
Hyperpar.sigma_w2 = 10^HP(end-1);
Hyperpar.Sigma_v = (10^HP(end))*eye(na);
