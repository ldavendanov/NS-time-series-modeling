function [theta_hat,P_theta,y_hat,other] = GSCTARMA_MAPtrajectory(y,na,nc,Hyperpar,options)
%--------------------------------------------------------------------------
% This function computes the MAP estimates of the parameter trajectories of
% a GSC-TARMA$(n_a,n_c)$ model based on a time series $y[t]$ and stochastic
...
%   P_theta   -> Parameter estimation error covariance ( 3D matrix : na+nc x na+nc x N )
...
global N naG ncG q muG sw Sv yG
N = length(y);
yG = y;
naG = na;                               % AR order
ncG = nc;                               % AR order
...
theta_hat.pred = other.xttm(1:na+nc,:);                 % One-step-ahead prediction of the parameter vector
P_theta.pred = other.Pttm(1:na+nc,1:na+nc,:);           % Covariance matrix of the parameter prediction error
y_hat.pred = y+other.ettm;                              % One-step-ahead signal prediction

% Extracting the results from filtering
theta_hat.filt = xtt(1:na+nc,:);                        % Filtered estimate of the parameter vector
P_theta.filt = Ptt(1:na+nc,1:na+nc,:);                  % Covariance matrix of the filtered parameter estimate
...
if strcmp(options.estim,'ks')
    ...
    theta_hat.smooth = xttN(1:na+nc,:);                 % Smoothed estimates of the parameter vector
    P_theta.smooth = PttN(1:na+nc,1:na+nc,:);           % Covariance matrix of the smoothed parameter estimates
...
%--------------------------------------------------------------------------
function F = stm(~)
global q muG naG ncG
F = [-muG; eye(q-1,q)];
F = kron(F,eye(naG+ncG));
...
max_del = min(ncG,t-1);
H(naG+(1:max_del)) = e(t-1:-1:t-max_del);

%--------------------------------------------------------------------------
function G = ncm(~)
global naG ncG q
G = eye(q,1);
G = kron(G,eye(naG+ncG));
